...
`dashboard.js` renders a rolling week (today + 6). Each day card has **two button-lanes** — **Lexie Activity** + **Strive Workout** — plus a **compact Meals line** beneath them, then the metric tiles on top and an "Also today" list for loose tasks (e.g. `log-food-today`). The contract for it:
...
- Three render states per lane: **planned** (chip → `cta_url`), **acknowledged** ("Rest day" | "No activity" + undo), **unplanned** (Plan deep-link + ack button). The two button-lanes writing `day-<dow>` don't collide — unique key is `(household_id, app, key)` and `app` differs.
- **Meals line (build 22 / v0.6.0; icons v0.6.1):** Lexie **also** publishes `key='meal-<dow>'` (same `app='lexie'`, `kind='task'`), `status='open'` when any slot set else `dismissed`. `title` carries **one icon-prefixed meal per line, newline-joined** (`🥣 …\n🍽️ …` — breakfast/lunch/dinner from `S.dayMeals`). `dashboard.js` reads it via `open('lexie','meal-<dow>')` and `mealLine()` **splits `title` on `\n`** → one `.dm-item` row per meal (icon associates each to its slot); a legacy dot-joined title with no `\n` renders as a single row. Empty → quiet `＋ Plan meals` link (→ `…/#meals`). **Deliberately no ack** (an undecided meal isn't a "gap" like a rest day). It shares Lexie's `day-`/`meal-` key namespace, so the **loose-task filter must exclude BOTH `day-*` and `meal-*`** or the rows leak into "Also today".
